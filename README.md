...
built to emulate real adversary activity and detect it across both network and host sensors.
...
internet. Attacks are launched from an isolated offensive VLAN against a victim network, and
...
| **40** | Attack Network | Kali Linux offensive host — isolated, no internet |
...
- **Nessus** — scheduled vulnerability scanning of the victim network
...
Ten real attacks mapped to MITRE ATT&CK, spanning Discovery, Initial Access, Execution,
...
AC Infinity cooling · wall-mounted server rack · JetKVM out-of-band access.

## The site

A multi-page static site (plain HTML / CSS / JavaScript, no build step) hosted on GitHub
Pages, covering the network diagram, hardware, software stack, attacks & detections, custom
detection rules, a build gallery, and full written documentation.

## Documentation

Written investigations and configuration references covering the Wazuh offline install,
interface & bridge mapping, a CVSS-scored vulnerability assessment, SIEM hunting queries,
firewall rules, the full network & hardware build, an Active Directory attack-chain case
study, and a risk-prioritization matrix.
